...
export default function SIndividualItem() {
  const { slug, title } = useParams(); // slug & title from URL (slug = product.slug)
  ...
  // primary: find by product.slug === slug
  // secondary: match by slugified title param === slugify(product.title)
  const product =
    products.find(
      (p) => String(p.slug || "").toLowerCase() === String(slug).toLowerCase()
    ) ||
    products.find(
      (p) => slugify(p.title || "") === String(title).toLowerCase()
    );
      ...
      <div className="max-w-6xl mx-auto px-4">
        ...
        <div className="text-sm text-gray-500 mb-4">
          ...
          {product.brand ? (
            <>
              <Link
                to={`/brands/${encodeURIComponent(slugify(product.brand))}`}
            ...
            <div className="mt-8">
              ...
              <div
                ...
                dangerouslySetInnerHTML={{
                  __html: product.description || product.short || "",
                }}
              />
            </div>
